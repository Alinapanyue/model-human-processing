...
# @MLEPORI EDIT
def print_metrics_to_csv(model_name, dataset_name,
                             metric_dict, metric_name,
                             data_parent_dir=c.METRICS_DIR):
    
    data_dir = pjoin(data_parent_dir, model_name, dataset_name)
    if not os.path.exists(data_dir):
        os.makedirs(data_dir)
    csv_file_path = pjoin(data_dir, metric_name + ".csv")
    ...
    writer.writerow(["subj", "dataset_name",
                    "metric_name", "stimulus", "layer", "value"])
            
    for stimulus, layer_dict in metric_dict.items():
        for layer, metric in layer_dict.items():

            stimulus = stimulus.split("/")[-1]
            newrow = [model_name, dataset_name,
                     metric_name, stimulus, layer, metric]
